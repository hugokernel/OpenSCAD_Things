...
@details This OpenSCAD script will generate a 2 part fit together packaging which connect together using simple lips.
...
use <2012_05_16-WIMU_Build_Rack_v0.7.scad>;//make_the_damn_thing(has_mouse_ears=false);
//*************Control*Variables****************
...
//internal dimensions
int_l_holder=112;//max length of the item to be held internally
int_b_holder=60.3;//max breadth of the item to be held internally
int_h_holder=32;//max height of the item to be held internally
int_r_holder=0.5;//radius of the corners of the device to be held internally
int_tol_holder=1;//distance between internal walls and edges of the item to be held internally, this allows for devices that may not be accurately measured or made and should generally be >0.5mm
int_r_tol_holder=0.1;//tolerance for the internal corners
curve_f_holder=3;//how many facets do the curved corners have?
wall_t_holder=2;//how thick are the vertical walls?
base_t_holder=2;//how thick is the base?
bottom_top_int_h_ratio=0.4;//how tall is the internal void of the bottom section relative to the top section (0.5 would give 2 halves of equal height)

$fn=curve_f_holder*4;//this is a special variable that makes the corners round to the requested number of facets

//lip dimensions
lip_h=2;//how tall are the interlocking lips?
...
//cutout variables
cutout_wall_thickness=1*xy_resolution;//+a_bit/2;//this'll give you through cutouts //how thick is the plastic left in the cutouts on the sides? set to 0 if you do not want any plastic left in these cutouts
...
north_cutout=0;//if 1 cutout on this side else none
n_cutout_c=0;//if 1 gives circle just using the _r value and ignoring the others, else gives rounded square
...
n_cutout_r=2;//radius of the corners of the cutout
n_cutout_e=4;//height of the bottom of the cutout above the bottom of the holder 
n_cutout_t=20;//translation along the face -left +right as you look at the face from outside
...
south_cutout2=1;//if 1 cutout on this side else none
s_cutout_c2=0;//if 1 gives circle just using the _r value and ignoring the others, else gives rounded square
s_cutout_l2=13.2+1;//length/tallness of the cutout
s_cutout_b2=8.5;//breadth of the cutout
s_cutout_h2=wall_t_holder+2*a_bit;//basically wall thickness plus a bit
s_cutout_r2=2;//radius of the corners of the cutout
s_cutout_e2=12.5+1.9;//height of the bottom of the cutout above the bottom of the holder 
s_cutout_t2=-7+2.8;//translation along the face -left +right
...
w_cutout_c=0;//if 1 gives circle just using the _r value and ignoring the others, else gives rounded square
w_cutout_l=13.1;//length/tallness of the cutout
w_cutout_b=8;//breadth of the cutout
w_cutout_h=wall_t_holder+2*a_bit;//basically wall thickness plus a bit
w_cutout_r=2;//radius of the corners of the cutout
w_cutout_e=13;//w_cutout_l+(-int_h_holder -lip_h)/2-int_tol_holder;//height of the bottom of the cutout above the bottom of the holder 
w_cutout_t=-8;//translation along the face -left +right

top_cutout=0;//top cutout
t_cutout_c=0;//if 1 gives circle just using the _r value and ignoring the others, else gives rounded square
...
b_cutout_c=0;//if 1 gives circle just using the _r value and ignoring the others, else gives rounded square
...
if (has_bottom==1){
	...
	//	}
		...
		if(has_device==1 && device_on_top==0){//a device resembling the electronics can be inserted here
			translate(v=[int_l_holder/2,int_b_holder/2,int_h_holder+base_t_holder+int_tol_holder])color([0,1,1,1]) rotate(a=[0,180,0])make_the_damn_thing(has_mouse_ears=false);
		}
...
if (has_top==1){
	...
	if(has_device==1 && device_on_top==1){	translate(v=[int_l_holder/2,-int_b_holder/2,+base_t_holder+int_tol_holder])color([1,0,1,1]) rotate(a=[0,0,180])make_the_damn_thing(has_mouse_ears=false);
	}
